        ...
        return new Promise( (resolve, reject) =>
        {
            if (name && log)
            {
                ...
                worker.on('exit', (code, signal) =>
                {
                    ...
                    .then( id =>
                    {
                        log.info('[PROBIOTIC] The new worker has been succesfully started with ID "%s"', id);
                    })
                    .catch( err => log.error('[PROBIOTIC] %s', err) );
                });
            }
            else
            {
...
module.exports =
{
    ...
    normalizeOptions: function(options)
    {
        ...
        if (options)
        {
            ...
            result.logger = module.exports.CONSOLE_LOGGER;
